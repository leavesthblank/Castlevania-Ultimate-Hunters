...
public class GrappleController {
    ...
    public GrappleController(int arrowId, int entityId, World world, Vec pos, int maxlen, int controllerid) {
        ...
        this.r = this.pos.sub(Vec.positionvec(entity))
            .length();
        ...
        if (this.attached) {
            if (entity != null) {
                if (true) {
                    ...
                    if (arrowpos.sub(playerpos.add(motion))
                        .length() > r) { // moving away
                     ...
                     * if (counter > 100) {
                     * counter = 0;
                     * GrappleMod.network.sendTo(new PlayerPosMessage(entity.getEntityId(), entity.posX, entity.posY,
                     * entity.posZ), (EntityPlayerMP) entity);
                     * }
                     */
                    // }
    ...
    public Vec dampenmotion(Vec motion, Vec forward) {
        ...
        return new Vec(
            newmotion.x * dampening + motion.x * (1 - dampening),
            newmotion.y * dampening + motion.y * (1 - dampening),
            newmotion.z * dampening + motion.z * (1 - dampening));
    }

    public void updateServerPos() {
        GrappleMod.network.sendToServer(
            new PlayerMovementMessage(
                this.entityId,
                this.entity.posX,
                this.entity.posY,
                this.entity.posZ,
